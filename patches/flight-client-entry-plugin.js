"use strict";
const __normalizePathSep = (p)=> (typeof p === "string" ? p.split(String.fromCharCode(92)).join("/") : p);
    ...
    async createActionAssets(compilation) {
        ...
        (0, _utils1.traverseModules)(compilation, (mod, _chunk, chunkGroup, modId)=>{
            ...
            if (chunkGroup.name && mod.request && modId && /next-flight-action-entry-loader/.test(mod.request)) {
                ...
                const entryName = __normalizePathSep(chunkGroup.name);
        ...
        for(let id in pluginState.serverActions){
            ...
            for(let name in action.workers){
                const targetLayer = action.layer[name] === _constants.WEBPACK_LAYERS.actionBrowser ? 'client' : 'server';
                const modId = pluginState.serverActionModules[name] && pluginState.serverActionModules[name][targetLayer];
                if (modId) {
        ...
        for(let id in pluginState.edgeServerActions){
            ...
            for(let name in action.workers){
                const targetLayer = action.layer[name] === _constants.WEBPACK_LAYERS.actionBrowser ? 'client' : 'server';
                const modId = pluginState.edgeServerActionModules[name] && pluginState.edgeServerActionModules[name][targetLayer];
                if (modId) {
                    action.workers[name] = modId;
                }
            }
